	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, substrate_node_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: substrate_node_runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer {
							...
							value: 100 * DOLLARS -
								(substrate_node_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop =>
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> substrate_node_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			Some(substrate_node_runtime::wasm_binary_unwrap()),
